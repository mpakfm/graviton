	...
	switch ($arParams['VIEW_MODE'])
	{
		...
		case 'TEXT':
			foreach ($arResult['SECTIONS'] as &$arSection)
			{
				...
				?><li id="<? echo $this->GetEditAreaId($arSection['ID']); ?>"><h2 class="bx_catalog_text_title">
                    <a href="<?=($arSection['UF_LINK'] != '' ? $arSection['UF_LINK'] : 'javascript:void(0);'); ?>"><? echo $arSection['NAME']; ?></a><?
				if ($arParams["COUNT_ELEMENTS"] && $arSection['ELEMENT_CNT'] !== null)
				{
					?><!-- <span>(<? echo $arSection['ELEMENT_CNT']; ?>)</span>--><?
				}
				?></h2>
                <?php if (!empty($arResult['ELEMENTS'][$arSection['ID']])) { ?>
                    <ul class="section-el-list">
                <?php foreach ($arResult['ELEMENTS'][$arSection['ID']] as $el) { ?>
