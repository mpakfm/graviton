...
))
{
	...
	$arResult["SECTIONS_COUNT"] = count($arResult["SECTIONS"]);
        ...
        $selected = array_merge(["IBLOCK_SECTION_ID"], $arParams['ELEMENT_FIELDS'], $arParams['ELEMENT_USER_FIELDS']);
        $stmt = CIBlockElement::GetList([], $filter, false, false, $selected);
        $elements = [];
        while ($el = $stmt->Fetch()) {
            if (!array_key_exists($el['IBLOCK_SECTION_ID'], $elements)) {
                $elements[$el['IBLOCK_SECTION_ID']] = [];
            }
