...
if ($this->startResultCache(false, ($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()))) {
    ...
    $ids            = null;
    $filterSections = [];
    if (array_key_exists('SECTIONS', $arParams) && !empty($arParams['SECTIONS'])) {
        foreach ($arParams['SECTIONS'] as $section) {
            if (is_int($section) && $section) {
                $filterSections[] = $section;
                continue;
            }
            $sectionFields = \Library\Tools\CacheSelector::getSectionByCode($arParams["IBLOCKS"][0], $section);
            if (!$sectionFields) {
                continue;
            }
            $filterSections[] = $sectionFields['ID'];
        }
        $filter = [
            ...
            "ACTIVE"            => "Y",
            "CHECK_PERMISSIONS" => "Y",
            "SECTION_ID"        => $filterSections,
        ];
        if ($arParams['IS_ACTIVE_DATE']) {
            $filter['ACTIVE_DATE'] = 'Y';
        }
        $stmt = CIBlockElement::GetList([], $filter, false, false, ['ID']);
        $ids  = [];
